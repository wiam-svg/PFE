...
// import {
//   Menu, Home, Bell, MapPin, LogOut, User, Wrench, ChevronDown, ChevronRight,
...
//               >
//                 <MapPin size={22} />
//                 <span className="ml-3">Voir la carte</span>
//               </Link>
//               <Link
//                 href="/logout"
//                 method="post"
...
//         >
//           <MapPin size={20} />
//           {isOpen && <span className="ml-3">Voir la carte</span>}
//         </Link>
//         <Link
//           href="/logout"
//           method="post"
    ...
    <>
      ...
      <div className="fixed top-0 left-0 w-full bg-green-900 text-white z-40 flex items-center justify-between px-4 py-3 shadow-md">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold">{t('agent_sidebar.title')}</h2>
        </div>
        <button
          onClick={toggleMobileMenu}
          className="p-2 rounded-md hover:bg-green-700"
      ...
      {mobileMenuOpen && (
        <div className="fixed top-14 left-0 right-0 bottom-0 bg-green-800 z-30 overflow-y-auto">
          <nav className="flex flex-col p-4 gap-3">
            ...
            >
              <Database size={22} />
              <span className="ml-3">{t('agent_sidebar.menu.dashboard')}</span>
            </Link>

            {/* Interventions submenu */}
            <div>
              <button
                onClick={() => toggleSubmenu('interventions')}
                className={`flex justify-between w-full items-center p-3 rounded-lg hover:bg-green-700 ${isActive('/agent/interventions') ? 'bg-green-700' : ''}`}
              >
                <div className="flex items-center">
                  <Wrench size={22} />
                  <span className="ml-3">{t('agent_sidebar.menu.interventions')}</span>
                </div>
                {activeSubmenu === 'interventions' ? <ChevronDown size={18} /> : <ChevronRight size={18} />}
              </button>
              ...
              {activeSubmenu === 'interventions' && (
                <div className="ml-4 mt-1 space-y-1 border-l-2 border-green-600 pl-3">
                  <Link
                    ...
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    {t('agent_sidebar.menu.in_progress')}
                  </Link>
                  <Link
                    href={route('interventions.terminees')}
                    className={`block p-3 rounded-md hover:bg-green-700 ${isActive('/interventions/terminee')}`}
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    {t('agent_sidebar.menu.completed')}
                  </Link>
                  <Link
                    href={route('interventions.rejetes')}
                    className={`block p-3 rounded-md hover:bg-green-700 ${isActive('/interventions/rejetee')}`}
            ...
            >
              <History size={22} />
              <span className="ml-3">{t('agent_sidebar.menu.history')}</span>
            </Link>
            ...
            >
              ...
              <span className="ml-3">{t('agent_sidebar.menu.notifications')}</span>
            </Link>
            ...
            <div className="mt-auto pt-6 border-t border-green-700">
              ...
              >
                <User size={20} />
                <span className="ml-3">{t('agent_sidebar.menu.profile')}</span>
              </Link>
              <Link
                href='/Carte'
                className="flex items-center p-3 rounded-lg hover:bg-green-700"
              ...
              >
                <MapPin size={22} />
                <span className="ml-3">{t('agent_sidebar.menu.view_map')}</span>
              </Link>
              <Link
                href="/logout"
                method="post"
              ...
              >
                <LogOut size={22} />
                <span className="ml-3">{t('agent_sidebar.menu.logout')}</span>
              </Link>
            </div>
          </nav>
        </div>
  ...
  const DesktopView = () => (
    <div className={`flex flex-col ${isOpen ? 'w-64' : 'w-20'} bg-green-800 text-white min-h-screen transition-all duration-300 sticky top-0`}>
      ...
      <div className="flex items-center justify-between p-4 bg-green-900 border-b border-green-700">
        {isOpen && (
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-semibold">{t('agent_sidebar.title')}</h2>
          </div>
        )}
        <button onClick={toggleSidebar} className="p-2 rounded-md hover:bg-green-700">
          <Menu size={24} />
      ...
      <nav className="flex flex-col p-3 gap-2 flex-grow overflow-y-auto">
        ...
        >
          <Database size={20} />
          {isOpen && <span className="ml-3">{t('agent_sidebar.menu.dashboard')}</span>}
        </Link>
        ...
        <div>
          ...
          >
            <div className="flex items-center">
              <Wrench size={20} />
              {isOpen && <span className="ml-3">{t('agent_sidebar.menu.interventions')}</span>}
            </div>
            {isOpen && (activeSubmenu === 'interventions' ? <ChevronDown size={16} /> : <ChevronRight size={16} />)}
          </button>
        ...
        >
          <History size={20} />
          {isOpen && <span className="ml-3">{t('agent_sidebar.menu.history')}</span>}
        </Link>
        ...
        >
          ...
          {isOpen && <span className="ml-3">{t('agent_sidebar.menu.notifications')}</span>}
        </Link>
      </nav>
      ...
      <div className="p-3 border-t border-green-700 space-y-2">
        ...
        >
          <User size={20} />
          {isOpen && <span className="ml-3">{t('agent_sidebar.menu.profile')}</span>}
        </Link>
        ...
        >
          <MapPin size={20} />
          {isOpen && <span className="ml-3">{t('agent_sidebar.menu.view_map')}</span>}
        </Link>
        <Link
          href="/logout"
          method="post"
        ...
        >
          <LogOut size={20} />
          {isOpen && <span className="ml-3">{t('agent_sidebar.menu.logout')}</span>}
        </Link>
      </div>
    </div>
  );

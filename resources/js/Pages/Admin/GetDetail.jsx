...
export default function GetDetail({ signalement }) {
    ...
    const statusColors = {
        [t('admin_dashboard.status_pending')]: "bg-yellow-100 text-yellow-800",
        [t('admin_dashboard.status_in_progress')]: "bg-blue-100 text-blue-800",
        [t('admin_dashboard.status_resolved')]: "bg-green-100 text-green-800",
        [t('admin_dashboard.status_rejected')]: "bg-red-100 text-red-800"
    };
            ...
            <div className="flex min-h-screen bg-gray-50">
                ...
                <div className="flex-1 w-full max-w-5xl mx-auto px-4 py-6">
                    ...
                    <div className="flex items-center mb-6">
                        ...
                        <h1 className="text-2xl font-bold">{t('admin_dashboard.report_details')}</h1>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            <div className="bg-white shadow rounded-lg overflow-hidden">
                                {signalement.image && (
                                    <div className="w-full h-64 bg-gray-200">
                                        <img
                                            src={`/storage/${signalement.image}`}
                                            alt={t('admin_dashboard.report_image')}
                                            className="w-full h-full object-cover"
                                        />
                                    </div>
                                )}
                                ...
                                <div className="p-6">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 text-sm text-gray-600">
                                        ...
                                        <div className="flex items-center">
                                            <MapPin size={16} className="mr-2" />
                                            <span>{signalement.adresse || t('admin_dashboard.not_specified')}</span>
                                        </div>
                                        <div className="flex items-center">
                                            <AlertCircle size={16} className="mr-2" />
                                            <span>{t('admin_dashboard.category1')}: {signalement.categorie?.nom}</span>
                                        </div>
                                        <div className="flex items-center">
                                            <User size={16} className="mr-2" />
                                            <span>{signalement.user?.prenom} {signalement.user?.nom}</span>
                            ...
                            <div className="bg-white shadow rounded-lg p-6">
                                <h3 className="text-lg font-semibold mb-4 flex items-center">
                                    <MessageSquare size={20} className="mr-2" />
                                    {t('admin_dashboard.comments')} ({signalement.commentaire.length})
                                </h3>
                                ...
                                {signalement.commentaire.length > 0 ? (
                                        ...
                                        {signalement.commentaire.map(comment => (
                                            <div key={comment.id} className="pb-4 border-b border-gray-100 last:border-0">
                                                <div className="flex items-center mb-2">
                                                    ...
                                                    <div>
                                                        <p className="font-medium">{comment.user.prenom} {comment.user.nom}</p>
                                                        <p className="text-xs text-gray-500">{t('admin_dashboard.days_ago', { count: 2 })}</p>
                                                    </div>
                                                </div>
                                                <p className="text-gray-700 pl-10">{comment.contenu}</p>
                                            </div>
                                    ...
                                    <div className="text-center py-8 text-gray-500">
                                        <MessageSquare size={24} className="mx-auto mb-2 opacity-50" />
                                        <p>{t('admin_dashboard.no_comments')}</p>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="space-y-6">
                            <div className="bg-white shadow rounded-lg p-6">
                                <h3 className="text-lg font-semibold mb-4">{t('admin_dashboard.additional_info')}</h3>
                                <p>{t('admin_dashboard.name')}: {signalement.user.nom}</p>
                                <p>{t('admin_dashboard.phone')}: {signalement.user.telephone}</p>
                                <p>{t('admin_dashboard.age')}: {signalement.user.age}</p>
                                <p>{t('admin_dashboard.email')}: {signalement.user.email}</p>
                                <p>{t('admin_dashboard.city')}: {signalement.user.ville}</p>

...
// const MapComponent = ({ signalements, selectedCategory, selectedCity}) => {
...
const MapComponent = ({ signalements, selectedCategory, selectedCity }) => {
      ...
      const marker = L.marker([signalement.latitude, signalement.longitude], { icon: DefaultIcon })
        .bindPopup(`
          <div class="popup-content">
            <h3>${signalement.titre}</h3>
            <p>${signalement.description}</p>
            <div><strong>${t('reports1.fields.city')}:</strong> ${signalement.ville}</div>
            <div><strong>${t('reports1.fields.category')}:</strong> ${signalement.categorie.nom}</div>
          </div>
        `);
  ...
  return (
    <div className="map-container">
      ...
      />
      <div className="map-info mt-2 text-sm text-gray-600">
        {t('reports1.results.count', { count: filteredSignalements.length })}
      </div>
    </div>
  );
};

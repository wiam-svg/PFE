    ...
    <AppLayout>
      <div className="flex min-h-screen bg-gray-50">
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          {/* Filters section */}
          <div className="bg-gray-100 p-4 rounded-lg mb-6">
            <h2 className="text-xl font-semibold mb-4">{t('reports.filters.title')}</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {/* Category filter */}
              <div>
                <label htmlFor="category-filter" className="block text-sm font-medium text-gray-700 mb-1">
                  {t('reports.filters.category.label')}
                </label>
                <select
                  id="category-filter"
                  value={selectedCategory}
                  ...
                  className="w-full p-2 border border-gray-300 rounded-md"
                >
                  <option value="">{t('reports.filters.category.all')}</option>
                  {categories.map((category) => (
                    <option key={category.id} value={category.id.toString()}>
                      {category.nom}
                    </option>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full p-2 border border-gray-300 rounded-md"
                >
                  <option value="">{t('reports.filters.city.all')}</option>
                  {cities.map((city) => (
                    <option key={city} value={city}>
                      {city}
                    </option>
          ...
          <div className="mb-4 text-sm">
            <span className="font-medium">
              {t('reports.results.count', { count: filteredSignalements.length })}
            </span>
          </div>
          
          {/* Map with filtered reports */}
          <div className="mb-8">
            <MapComponent 
              signalements={signalements} 
              selectedCategory={selectedCategory}
          ...
          {/* Reports list */}
          <div>
            <h2 className="text-xl font-semibold mb-4">{t('reports.list.title')}</h2>
            
            {filteredSignalements.length === 0 ? (
              <div className="bg-gray-50 p-4 rounded-md text-center text-gray-500">
                {t('reports.results.none')}
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredSignalements.map((signalement) => (
                  <div key={signalement.id} className="bg-white shadow-md rounded-lg overflow-hidden">
                    <div className="p-4">
                      ...
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="bg-green-100 text-green-800 px-2 py-1 rounded">
                          {signalement.categorie?.nom || t('reports.list.uncategorized')}
                        </span>
                      </div>
                    </div>
                  </div>

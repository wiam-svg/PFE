...
//                   <SignalementDetails signalement={selectedSignalement} />
//                 </div>
  ...
  return (
    <AppLayout>
      <div className="flex min-h-screen bg-gray-50">
        ...
        <div className="flex-1 p-6 max-w-6xl mx-auto">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-800 mb-6">{t('agent_signalements.title')}</h1>
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 flex-wrap">
              ...
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder={t('agent_signalements.search_placeholder')}
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm"
                />
              ...
              <div className="flex items-center space-x-3 bg-white p-2 rounded-lg shadow-sm">
                ...
                <select
                  ...
                  className="block px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm"
                >
                  <option value="">{t('agent_signalements.filters.all_categories')}</option>
                  {categories.map((cat, index) => (
                    <option key={index} value={cat}>{cat}</option>
                  ))}
                </select>
              ...
              <div className="flex items-center space-x-2">
                ...
                <button
                  ...
                  className="px-4 py-2 text-sm bg-gray-100 border border-gray-300 rounded-md shadow-sm hover:bg-gray-200 transition"
                >
                  {t('agent_signalements.filters.sort.' + (sortOrder === 'desc' ? 'new_to_old' : 'old_to_new'))}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-blue-50 border-l-4 border-blue-500 p-4 mb-6 rounded-md shadow-sm">
            <div className="flex">
              ...
              <div className="ml-3">
                <p className="text-sm text-blue-700">
                  {t('agent_signalements.info_bar', { count: filteredSignalements.length })}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm overflow-hidden">
            {sortedSignalements.length > 0 ? (
                  ...
                  <div className="p-4 flex flex-col md:flex-row md:items-center md:justify-between">
                    <div className="flex-1">
                      <div className="flex items-center mb-2">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          ...
                          {s.resolution_status === 'Terminé'
                            ...
                            : <AlertTriangle size={14} className="mr-1" />}
                          {s.resolution_status === 'Terminé' 
                            ? t('agent_signalements.status.completed') 
                            : t('agent_signalements.status.pending')}
                        </span>
                        {s.signalement?.categorie?.nom && (
                          <span className="ml-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-gray-800">
                            {s.signalement.categorie.nom}
                      ...
                      <div className="mt-2 text-xs text-gray-500">
                        {t('agent_signalements.labels.address')}: {s.signalement.adresse}
                      </div>
                      <div className="mt-1 text-xs text-gray-400">
                        {t('agent_signalements.details_modal.created_at', { 
                          date: new Date(s.signalement?.created_at).toLocaleDateString() 
                        })}
                      </div>
                    </div>
                    <div className="flex mt-4 md:mt-0 space-x-3">
                      ...
                      >
                        <Eye size={16} className="mr-2" />
                        {t('agent_signalements.buttons.details')}
                      </button>
                      {s.resolution_status !== 'Terminé' && (
                        <button
                          onClick={() => router.get(`/agent/intervention/${s.id}/editStatut?source=encours`)}
                        ...
                        >
                          <CheckCircle size={16} className="mr-2" />
                          {t('agent_signalements.buttons.mark_completed')}
                        </button>
                      )}
                    </div>
                  </div>
              ...
              <div className="p-8 text-center">
                <p className="text-gray-500">{t('agent_signalements.no_results')}</p>
              </div>
            )}
          </div>
              ...
              <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
                <div className="flex justify-between items-center p-4 border-b">
                  <h2 className="text-xl font-semibold text-gray-800">
                    {t('agent_signalements.details_modal.title')}
                  </h2>
                  <button
                    onClick={handleCloseDetails}
                    className="p-1 rounded-full hover:bg-gray-100 transition"
                ...
                <div className="p-4 border-t bg-gray-50 flex justify-end">
                  <button
                    ...
                    className="px-4 py-2 bg-white border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 transition"
                  >
                    {t('agent_signalements.buttons.close')}
                  </button>
                </div>
              </div>
            </div>

...
const InterventionsRejetes = ({ interventions }) => {
  ...
  const MobileView = () => (
    <div className="flex flex-col bg-gray-50 min-h-screen">
      <div className="p-4 flex-1">
        <div className="mb-4">
          <div className="flex justify-between items-center mb-3">
            <p className="text-xs text-gray-500">{t('InterventionsRejetes.manage')}</p>
            <button 
              onClick={() => setShowFilters(!showFilters)}
              className="bg-gray-100 p-2 rounded-lg text-gray-600 hover:bg-gray-200 flex items-center"
            >
              <Filter size={16} className="mr-1" />
              <span className="text-xs">{t('InterventionsRejetes.filters')}</span>
            </button>
          </div>
            ...
            <div className="bg-white rounded-lg shadow-md p-3 mb-3 border border-gray-200">
              <div className="mb-3">
                <div className="flex items-center bg-gray-50 rounded-lg px-3 py-2 border border-gray-200">
                  ...
                  <input
                    type="text"
                    placeholder={t('InterventionsRejetes.searchPlaceholder')}
                    className="bg-transparent border-0 flex-1 focus:outline-none text-sm"
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                  />
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  <span className="text-xs text-gray-500 mr-2">{t('InterventionsRejetes.sortBy')}:</span>
                  <select
                    className="text-xs bg-gray-50 border border-gray-200 rounded-md px-2 py-1"
                    value={sortField}
                    onChange={(e) => {
                  ...
                  >
                    <option value="priorite">{t('InterventionsRejetes.priority')}</option>
                    <option value="dateDebut">{t('InterventionsRejetes.startDate')}</option>
                    <option value="dateFin">{t('InterventionsRejetes.endDate')}</option>
                    <option value="resolution_status">{t('InterventionsRejetes.status')}</option>
                  </select>
                  <button 
                    onClick={() => setSortDirection(sortDirection === 'asc' ? 'desc' : 'asc')}
                    className="ml-2 bg-gray-100 p-1 rounded-md"
        ...
        {searchTerm && (
          <div className="bg-blue-50 rounded-lg p-2 mb-3 text-xs text-blue-700 flex items-center justify-between">
            <span>
              {t('InterventionsRejetes.matchingInterventions', {
        ...
        <div className="space-y-3">
          {sortedInterventions.length === 0 ? (
            <div className="bg-white rounded-xl p-6 shadow-md flex flex-col items-center justify-center text-center">
              <AlertCircle size={32} className="text-gray-400 mb-2" />
              <p className="text-gray-500 font-medium">{t('InterventionsRejetes.noRejectedInterventions')}</p>
              <p className="text-gray-400 text-xs mt-1">{t('InterventionsRejetes.willAppearHere')}</p>
            </div>
          ) : (
            sortedInterventions.map((intervention) => (
              <div key={intervention.id} className="bg-white rounded-xl shadow-md p-4 border border-gray-200">
                <div className="flex justify-between items-start mb-3">
                  ...
                  {intervention.priorite > 5 ? (
                    <span className="px-3 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-800">
                      {t('InterventionsRejetes.high')}
                    </span>
                  ) : (
                    <span className="px-3 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-800">
                      {t('InterventionsRejetes.medium')}
                    </span>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs text-gray-500 mb-4">
                  <div>
                    <p className="text-gray-400">{t('InterventionsRejetes.start')}:</p>
                    <p>{intervention.dateDebut}</p>
                  </div>
                  <div>
                    <p className="text-gray-400">{t('InterventionsRejetes.end')}:</p>
                    <p>{intervention.dateFin}</p>
                  </div>
                </div>
                ...
                <div className="flex space-x-2">
                  ...
                  >
                    <Eye size={14} className="mr-1" />
                    {t('InterventionsRejetes.details')}
                  </button>
                </div>
              </div>
            ))
        ...
        {sortedInterventions.length > 0 && (
          <div className="mt-4 bg-white p-3 rounded-lg shadow-sm border border-gray-200">
            <p className="text-sm text-gray-600 text-center">
              {t('InterventionsRejetes.totalRejected', { count: sortedInterventions.length })}
            </p>
          </div>
        )}
      </div>
  ...
  // Vue pour les ordinateurs de bureau
  const DesktopView = () => (
    <div className="flex bg-gray-50 min-h-screen">
      <div className="flex-1 p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-bold text-gray-800">{t('InterventionsRejetes.title')}</h1>
              <p className="text-sm text-gray-500 mt-1">{t('InterventionsRejetes.subtitle')}</p>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-md p-4 mb-6 flex items-center justify-between border border-gray-200">
            <div className="flex items-center flex-1">
              <div className="flex items-center bg-gray-50 rounded-lg px-3 py-2 border border-gray-200 flex-1 max-w-xl">
                ...
                <input
                  type="text"
                  placeholder={t('InterventionsRejetes.searchPlaceholderDesktop')}
                  className="bg-transparent border-0 flex-1 focus:outline-none"
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                />
            ...
            <div className="flex items-center space-x-3">
              <div className="flex items-center">
                <span className="text-sm text-gray-500 mr-2">{t('InterventionsRejetes.sortBy')}:</span>
                <select
                  className="text-sm bg-gray-50 border border-gray-200 rounded-md px-3 py-2"
                  value={sortField}
                  onChange={(e) => {
          ...
          {searchTerm && (
            <div className="bg-blue-50 rounded-lg p-3 mb-6 text-sm text-blue-700 flex items-center justify-between">
              <span>
                {t('InterventionsRejetes.matchingInterventionsDesktop', {
                  count: sortedInterventions.length,
                  searchTerm: searchTerm
                })}
              </span>
          ...
          <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200">
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead>
                  <tr className="bg-gradient-to-r from-gray-100 to-gray-50">
                    <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-gray-700 uppercase">
                      <div className="flex items-center cursor-pointer" onClick={() => handleSort('resolution_status')}>
                        {t('InterventionsRejetes.status')}
                        {getSortIcon('resolution_status')}
                      </div>
                    </th>
                    <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-gray-700 uppercase">
                      <div className="flex items-center cursor-pointer" onClick={() => handleSort('signalement.titre')}>
                        {t('InterventionsRejetes.reportTitle')}
                        {getSortIcon('signalement.titre')}
                      </div>
                    </th>
                    <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-gray-700 uppercase">
                      <div className="flex items-center cursor-pointer" onClick={() => handleSort('signalement.categorie.nom')}>
                        {t('InterventionsRejetes.category')}
                        {getSortIcon('signalement.categorie.nom')}
                      </div>
                    </th>
                    <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-gray-700 uppercase">
                      <div className="flex items-center cursor-pointer" onClick={() => handleSort('dateDebut')}>
                        {t('InterventionsRejetes.startDate')}
                        {getSortIcon('dateDebut')}
                      </div>
                    </th>
                    <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-gray-700 uppercase">
                      <div className="flex items-center cursor-pointer" onClick={() => handleSort('dateFin')}>
                        {t('InterventionsRejetes.endDate')}
                        {getSortIcon('dateFin')}
                      </div>
                    </th>
                    <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-gray-700 uppercase">
                      <div className="flex items-center cursor-pointer" onClick={() => handleSort('priorite')}>
                        {t('InterventionsRejetes.priority')}
                        {getSortIcon('priorite')}
                      </div>
                    </th>
                    <th scope="col" className="px-6 py-4 text-left text-xs font-medium text-gray-700 uppercase">
                      {t('InterventionsRejetes.actions')}
                    </th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {sortedInterventions.length === 0 ? (
                    <tr>
                      <td colSpan="8" className="py-10 text-center">
                        <div className="flex flex-col items-center justify-center">
                          <AlertCircle size={40} className="text-gray-400 mb-3" />
                          <p className="text-gray-500 font-medium">{t('InterventionsRejetes.noRejectedFound')}</p>
                          <p className="text-gray-400 text-sm">{t('InterventionsRejetes.willAppearHere')}</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
            ...
            {sortedInterventions.length > 0 && (
              <div className="bg-white px-6 py-4 border-t border-gray-200">
                <p className="text-sm text-gray-600 text-center">
                  {t('InterventionsRejetes.totalRejectedDesktop', { count: sortedInterventions.length })}
                </p>
              </div>
            )}
          </div>

...
const Historique = ({ interventions }) => {
  ...
  const MobileView = () => (
    <div className="p-4 w-full">
      ...
      {interventions.length === 0 ? (
        <div className="p-8 bg-white rounded-lg shadow text-center">
          <p className="text-gray-600">{t('historique.no_interventions')}</p>
        </div>
      ) : (
        <div className="space-y-4">
          {interventions.map((intervention) => (
            <div key={intervention.id} className="p-4 bg-white shadow rounded-xl border-l-4 border-green-800">
              ...
              {intervention.solution_photo && (
                <img
                  src={`/storage/${intervention.solution_photo}`}
                  alt={t('historique.solution_photo_alt')}
                  className="w-full rounded-lg mb-3 object-cover h-48"
                />
              )}
              ...
              <div className="mb-3">
                <p className="text-gray-700 text-sm mb-1">
                  <span className="font-medium">{t('historique.description')}:</span> {intervention.signalement.description}
                </p>
                <div className="flex items-center text-sm text-gray-700 mb-1">
                  <MapPin className="w-4 h-4 mr-1 text-gray-500" />
                  <span>{intervention.signalement.adresse}, {intervention.signalement.ville}</span>
              ...
              <div className="space-y-1 mb-3 text-xs text-gray-500">
                <div className="flex items-center">
                  <Wrench className="w-3 h-3 mr-1" />
                  <span>{t('historique.start_date')}: {intervention.dateDebut}</span>
                </div>
                <div className="flex items-center">
                  <CheckCircle className="w-3 h-3 mr-1" />
                  <span>{t('historique.end_date')}: {intervention.dateFin}</span>
                </div>
                <div className="flex items-center">
                  <Clock className="w-3 h-3 mr-1" />
                  <span>{t('historique.intervention_date')}: {new Date(intervention.updated_at).toLocaleDateString()}</span>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1 mb-3">
                <p className="text-sm">
                  <span className="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs">
                    {t('historique.intervention')}: {intervention.resolution_status}
                  </span>
                </p>
                <p className="text-sm">
                  <span className="px-2 py-1 bg-orange-100 text-orange-800 rounded-full text-xs">
                    {t('historique.report')}: {intervention.signalement.statut}
                  </span>
                </p>
              </div>
              ...
              <button
                ...
                className="w-full mt-2 flex items-center justify-center gap-2 py-2 bg-green-800 text-white text-sm rounded-md hover:bg-green-700 transition"
              >
                <FileDown className="w-4 h-4" /> {t('historique.pdf_button')}
              </button>
            </div>
          ))}
        </div>
  ...
  const DesktopView = () => (
    <div className="p-6 w-full">
      ...
      {interventions.length === 0 ? (
        <div className="p-12 bg-white rounded-lg shadow-md text-center">
          <p className="text-gray-600 text-lg">{t('historique.no_interventions')}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {interventions.map((intervention) => (
              ...
              <div className="p-6">
                ...
                <div className="mt-6 flex justify-end">
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-6 py-2 bg-green-800 text-white rounded-md hover:bg-green-700 transition"
                  >
                    <FileDown className="w-4 h-4" /> {t('historique.download_pdf')}
                  </button>
                </div>
              </div>
            </div>

...
export default function EditIntervention({ intervention }) {
  ...
  const { data, setData, post, processing, errors } = useForm({
    ...
    dateDebut: intervention.dateDebut || '',
    dateFin: intervention.dateFin || '',
    solution_photo: intervention.solution_photo || null,
    resolution_status: intervention.resolution_status || 'terminé',
  });
  ...
  return (
    <AppLayout>
      <div className="flex">
        <AgentSidebar />
        <div className="p-6 max-w-xl mx-auto bg-white rounded shadow">
          <h1 className="text-2xl font-bold mb-4">{t('EditIntervention.title')}</h1>
          <form onSubmit={handleSubmit}>
            <div className="mb-4">
              <label>{t('EditIntervention.actionDescription')}:</label>
              <textarea
                value={data.description_action}
                onChange={(e) => setData('description_action', e.target.value)}
                className="w-full border p-2"
                placeholder={t('EditIntervention.describeAction')}
              />
              {errors.description_action && (
                <div className="text-red-500">{errors.description_action}</div>
              )}
            ...
            <div className="mb-4">
              <label>{t('EditIntervention.startDate')}:</label>
              <input
                type="date"
                value={data.dateDebut}
                onChange={(e) => setData('dateDebut', e.target.value)}
            ...
            <div className="mb-4">
              <label>{t('EditIntervention.endDate')}:</label>
              <input
                type="date"
                value={data.dateFin}
                onChange={(e) => setData('dateFin', e.target.value)}
                ...
                <label htmlFor="image" className="cursor-pointer">
                  {imagePreview ? (
                    <div className="flex justify-center">
                      <img
                        src={imagePreview}
                        alt={t('EditIntervention.preview')}
                        className="h-40 object-contain"
                      />
                    </div>
                  ) : (
                    <div className="flex flex-col items-center justify-center py-5">
                      <Camera className="h-12 w-12 text-gray-400 mb-2" />
                      <p className="text-gray-500">{t('EditIntervention.addPhoto')}</p>
                    </div>
                  )}
                </label>
              </div>
